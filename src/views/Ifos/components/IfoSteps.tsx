  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Stake UBUT in IFO pool')}
      </Heading>
      <Box>
        <Text color="textSubtle" small>
          {t(
            'The maximum amount of UBUT user can commit to the Public Sale, is equal to the average UBUT balance in the IFO UBUT pool prior to the IFO. Stake more UBUT to increase the maximum UBUT you can commit to the sale. Missed this IFO? You can keep staking in the IFO UBUT Pool to join the next IFO sale.',
          )}
        </Text>
      ...
      {hasProfile && (
        <SmallStakePoolCard borderRadius="default" p="16px">
          <FlexGap justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="16px">
            ...
            {isVaultApproved ? (
              <Button onClick={onPresentStake}>{t('Stake')} UBUT</Button>
            ) : (
...
const Step2 = ({ hasProfile, isLive, isCommitted }: { hasProfile: boolean; isLive: boolean; isCommitted: boolean }) => {
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t('When the IFO sales are live, you can “commit” your UBUT to buy the tokens being sold.')} <br />
      </Text>
      ...
      case 3:
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small>
              {t(
                'After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent UBUT tokens will be returned to your wallet.',
              )}
            </Text>
          </CardBody>
        )

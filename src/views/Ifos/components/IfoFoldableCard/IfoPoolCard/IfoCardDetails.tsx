...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of UBUT up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max UBUT entry is capped by minimum between your average UBUT balance in the IFO UBUT pool, or the pool’s hard cap. To increase the max entry, Stake more UBUT into the IFO UBUT pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max UBUT entry is capped by your average UBUT balance in the IFO UBUT pool. To increase the max entry, Stake more UBUT into the IFO UBUT pool',
        )
      : t(
          'For the unlimited sale, Max UBUT entry is capped by your average UBUT balance in the IFO UBUT pool. To increase the max entry, Stake more UBUT into the IFO UBUT pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. UBUT entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('UBUT to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
